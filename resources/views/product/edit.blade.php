            ...
            <div class="bank-account clearfix text-left">
                ...
                <div class="col-md-6 lpadding">
                    ...
                    <div class="form-group{{ $errors->has('category_id') ? ' has-error' : '' }}">
                        ...
                        <!-- <select multiple="multiple" id="myMulti" name="category_id[]" multiple>
                            ...
                            <?php  $getCategory = explode(',',$data->category_id); ?>
                        <select class="form-control" name="category_id[]" multiple>
            \                      @foreach($category as $key =>$value)
                            <option value="{{$value->id}}" @if(in_array($value->id,$getCategory))selected @endif>{{$value->name}}</option>
                            @endforeach
                        </select>
